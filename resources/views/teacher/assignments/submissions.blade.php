    ...
    <div class="pt-10 pb-20" x-data="{ selected: [] }">
        ...
        <div class="mt-9 flex items-center gap-2.5 flex-wrap">
            @foreach ($filters as $f)
                <a href="{{ route('assignments.submissions', array_merge(['assignment' => $assignment], request()->query(), ['filter' => $f['key']])) }}"
            ...
            <div class="ml-auto flex items-center gap-3.5" x-show="selected.length === 0" x-transition>
                ...
                @foreach (['release' => 'Release all', 'hide' => 'Hide all'] as $action => $label)
                    <form method="POST" action="{{ route('assignments.similarity-release.bulk', $assignment) }}"
        ...
        <form method="GET" action="{{ route('assignments.submissions', $assignment) }}" class="mt-4 flex flex-wrap gap-2.5">
            ...
            @if ($search || $sort !== 'score')
                <a href="{{ route('assignments.submissions', ['assignment' => $assignment, 'filter' => $activeFilter]) }}" class="text-[13px] font-medium text-slate-800 hover:text-ink self-center">Clear</a>
            @endif
        </form>
            ...
            @forelse ($rows as $row)
                ...
                <div class="grid grid-cols-[28px_minmax(0,1.9fr)_104px_minmax(150px,1.35fr)_minmax(0,1fr)_auto] items-center gap-4 px-6 py-4 hover:bg-slate-50">
                    ...
                    <div class="text-right flex items-center justify-end gap-2">
                        ...
                        <a href="{{ route('submissions.show', $row->submission) }}"
                ...
                <div x-show="notesOpen" x-cloak x-transition class="px-6 pb-5 pt-1">
                    <div class="max-w-[560px] bg-slate-50 border border-slate-200 rounded-sm p-4">
                        @include('partials.note-thread', ['submission' => $row->submission, 'prefix' => ''])
                    </div>
                </div>
                </div>
            @empty
