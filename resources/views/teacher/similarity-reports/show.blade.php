        ...
        <div class="mt-6 flex flex-wrap gap-7 items-start">
            ...
            <div class="flex-1 min-w-[320px]">
                ...
                <div class="mt-5 grid gap-5" style="grid-template-columns: repeat(auto-fit, minmax(320px, 1fr));">
                    @include('partials.similarity-source-card', ['submission' => $focus, 'side' => 'A', 'report' => $report, 'prefix' => ''])
                    @if ($report->isWebSource())
                        @include('partials.similarity-web-source-card', ['report' => $report])
                    @else
                        @include('partials.similarity-source-card', ['submission' => $counterpart, 'side' => 'B', 'report' => $report, 'prefix' => ''])
                    @endif
                </div>
            </div>
            ...
            <div class="w-full sm:w-[300px] shrink-0 sticky top-[84px] grid gap-5">
                ...
                <div class="bg-white border border-slate-300 rounded-sm p-5">
                    ...
                    <div class="mt-4.5 grid gap-2">
                        <form method="POST" action="{{ route('similarity-reports.update-status', ['similarityReport' => $report, 'for' => $focus->id]) }}">
                            @csrf
                            @method('PATCH')
                    ...
                    <div class="px-5">
                        ...
                        <div class="mt-3 grid gap-2.5">
                            @foreach ($otherMatches as $match)
                                <a href="{{ route('similarity-reports.show', ['similarityReport' => $match->id, 'for' => $match->for]) }}"
