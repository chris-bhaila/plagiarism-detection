<x-app-layout max-width="narrow">
    <div class="pt-10 pb-20">
        <a href="{{ route('admin.semesters.show', $course->semester_id) }}" class="text-[12.5px] text-slate-800 hover:text-ink">
            &larr; Back to {{ $course->semester->label() }}
        </a>
        ...
        <div class="mt-7 bg-white border border-slate-300 rounded-sm p-6">
            <form method="POST" action="{{ route('admin.courses.update', $course) }}" class="grid gap-5" x-data="{
                    faculties: @js($faculties->map(fn ($f) => ['id' => $f->id, 'name' => $f->name, 'semesters' => $f->semesters->map(fn ($s) => ['id' => $s->id, 'number' => $s->number])])),
                    facultyId: null,
                    semesterId: {{ old('semester_id', $course->semester_id) ? (int) old('semester_id', $course->semester_id) : 'null' }},
                    get semesters() {
                ...
                <div>
                    ...
                    <x-text-input id="name" type="text" name="name" :value="old('name', $course->name)" required autofocus />
                ...
                <div>
                    ...
                    <select id="teacher_id" name="teacher_id"
                        class="w-full box-border bg-white border border-slate-500 rounded-sm px-3.5 py-2.5 text-[14.5px] text-ink focus:border-navy-light focus:outline-none">
                        <option value="" @selected(old('teacher_id', $course->teacher_id) === null)>Unassigned</option>
                        @foreach ($teachers as $teacher)
                            <option value="{{ $teacher->id }}" @selected((int) old('teacher_id', $course->teacher_id) === $teacher->id)>{{ $teacher->name }}</option>
                        @endforeach
                    </select>
