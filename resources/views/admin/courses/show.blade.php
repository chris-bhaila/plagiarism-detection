        ...
        <div class="mt-2.5 flex items-end justify-between gap-6 flex-wrap">
            ...
            <div class="flex items-center gap-2">
                ...
                <form method="POST" action="{{ route('admin.courses.destroy', $course) }}"
                      onsubmit="return confirm('Delete \'{{ $course->name }}\'? This also deletes its assignments, submissions, and similarity reports.');">
                    @csrf
                    @method('delete')
                    <button type="submit" class="text-[12.5px] font-semibold px-3.5 py-1.5 rounded-sm border border-slate-500 text-danger-ink bg-white hover:bg-danger-bg hover:border-danger-border">
                        Delete course
                    </button>
                </form>
            </div>
        ...
        <div class="mt-9 grid gap-8" style="grid-template-columns: repeat(auto-fit, minmax(380px, 1fr));">
            <div>
                ...
                <div x-show="creating" x-cloak x-transition class="mt-4 bg-white border border-slate-300 rounded-sm p-6">
                    <form method="POST" action="{{ route('admin.assignments.store') }}" class="grid gap-5" enctype="multipart/form-data" @submit="submitting = true">
                        @csrf
